...
RestrictionElement.prototype.description = function (definitions, xmlns) {
    ...
    if (desc && this.$base) {
        var type = splitQName(this.$base), typeName = type.name, ns = (xmlns && xmlns[type.prefix]) || definitions.xmlns[type.prefix], schema = definitions.schemas[ns], typeElement = schema &&
...
WSDL.prototype._processNextInclude = function (includes, callback) {
    var self = this, include = includes.shift(), options;
    if (!include)
        return callback();
    var includePath;
...
WSDL.prototype._parse = function (xml) {
    var self = this, p = sax.parser(true), stack = [], root = null, types = null, schema = null, schemaAttrs = null, options = self.options;
    p.onopentag = function (node) {
        var nsName = node.name;
        var attrs = node.attributes;
        var top = stack[stack.length - 1];
...
function open_wsdl(uri, options, callback) {
    ...
    fetch(uri).then(function (r) {
        r.text().then(function (definition) {
            wsdl = new WSDL(definition, uri, options);
            WSDL_CACHE[uri] = wsdl;
            wsdl.WSDL_CACHE = WSDL_CACHE;
            wsdl.onReady(callback);
